...
def _auth_css() -> str:
    ...
    .lnx-auth-shell{position:relative;z-index:2;width:38%;min-height:100vh;background:#fff;padding:clamp(46px,7vh,86px) clamp(46px,5vw,96px);display:flex;flex-direction:column;justify-content:center;box-sizing:border-box}
    ...
    .lnx-auth-title{margin:54px 0 28px;font-size:15px;font-weight:600;color:#17213F}
    .lnx-auth-links{margin-top:22px;text-align:center;color:#67708A;font-size:15px;line-height:1.5}.lnx-auth-links a{color:#12235F!important;text-decoration:none!important;font-weight:700}
    .lnx-auth-links .trial{display:block;margin-top:2px;font-weight:800}
    .lnx-back{display:inline-flex;margin-top:18px;color:#67708A!important;text-decoration:none!important;font-size:14px}
    .block-container>[data-testid="stVerticalBlock"]{width:38%!important;min-height:100vh!important;padding:0!important;margin:0!important;position:relative!important;z-index:3!important;background:transparent!important}
    .block-container>[data-testid="stVerticalBlock"]>[data-testid="stElementContainer"]{width:100%!important}
    .st-key-lnx_auth_form{width:calc(38vw - 92px)!important;max-width:500px!important;margin-left:clamp(46px,5vw,96px)!important;margin-top:calc(50vh - 160px)!important;position:relative!important;z-index:4!important;background:transparent!important}
    ...
    .lnx-auth-caption{margin-top:18px;color:#6E778D;font-size:14px;line-height:1.45}
...
def _shell(mode: str) -> None:
    ...
    heading = {
        ...
        "request": "Comece seu teste grátis",
        "invite": "Ative seu convite",
        "recovery": "Recupere seu acesso",
    }[mode]
    caption = {
        "login": "Entre com seu e-mail e senha para acessar o LicitaNexo.",
        "request": "7 dias grátis, sem cartão. Depois você decide se deseja continuar.",
                ...
                if st.form_submit_button("Solicitar acesso", width="stretch"):
                    ...
                    try:
                        ...
                        if risk.get("outcome") == "review":
                            st.success("Solicitação recebida. A equipe fará uma validação rápida antes da liberação.")
                        else:
