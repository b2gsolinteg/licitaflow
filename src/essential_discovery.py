...
def _apply_styles() -> None:
    st.markdown(
        ...
        [data-testid="stMain"] .block-container .stButton button[kind="primary"],[data-testid="stMain"] .block-container .stDownloadButton button[kind="primary"]{background:#2D5FE8 !important;color:#FFFFFF !important;border:1px solid #2D5FE8 !important;box-shadow:0 4px 10px rgba(45,95,232,.18) !important;}
        ...
        [data-testid="stMain"] .block-container .stButton button[kind="primary"]:hover{background:#244FCB !important;border-color:#244FCB !important;}
    ...
    for start_index in range(0, len(portals), 3):
        ...
        for col, portal_name in zip(cols, portals[start_index:start_index + 3]):
            ...
            with col.container(border=True):
                ...
                if st.button("Ver licitações", key=f"portal_{portal_name}", type="primary", width="stretch"):
    ...
    for start_index in range(0, len(counts), 3):
        ...
        for col, row in zip(cols, counts[start_index:start_index + 3]):
            ...
            with col.container(border=True):
                ...
                if st.button("Ver licitações", key=f"modality_{label}", type="primary", width="stretch"):
