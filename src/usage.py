...
class UsageService:
    ...
    def ensure_schema(self):
        with self.connect() as conn:
            ...
            """)
            conn.execute("""
                INSERT OR IGNORE INTO usage_settings(key,value)
    ...
    def default_analysis_limit(self):
        try:
            return max(0, int(float(self._setting(
                "essential_analysis_monthly_limit", self.DEFAULT_ANALYSIS_LIMIT
            ))))
    ...
    def default_estimated_cost_cents(self):
        try:
            return max(0.0, float(self._setting(
                "estimated_analysis_cost_cents", 0
            )))
        except (TypeError, ValueError):
            return 0.0
    ...
    def set_company_policy(self, company_id, analysis_limit=None, estimated_cost_cents=None):
    ...
    def month_usage(self, company_id, event_type="analysis"):
        with self.connect() as conn:
            ...
            """, (str(company_id), str(event_type))).fetchone()
        return {"units": int(row["units"] or 0), "cost_cents": float(row["cost"] or 0)}
    ...
    def record(self, company_id, user_id, event_type, units=1, metadata=None, estimated_cost_cents=None):
        units = max(1, int(units))
        if estimated_cost_cents is None:
            estimated_cost_cents = (
                self.company_policy(company_id)["estimated_analysis_cost_cents"] * units
                if event_type == "analysis" else 0
            )
        ...
        with self.connect() as conn:
            ...
            """, (
                str(company_id), str(user_id or ""), str(event_type), units,
                max(0.0, float(estimated_cost_cents)), payload[:4000],
            ))

    def record_analysis(self, company_id, user_id, filename="", pages=0):
        self.record(
            company_id, user_id, "analysis", 1,
            {"filename": str(filename)[:180], "pages": int(pages or 0)}
        )

    def record_search(self, company_id, user_id, keyword=""):
        self.record(
            company_id, user_id, "search", 1,
            {"keyword": str(keyword or "")[:180]}, estimated_cost_cents=0
        )
    ...
    def admin_summary(self):
        with self.connect() as conn:
            rows = conn.execute("""
                ...
                WHERE u.created_at >= datetime('now','start of month')
                  AND u.created_at < datetime('now','start of month','+1 month')
        ...
        for row in rows:
            ...
            result.append({
                ...
                "estimated_cost_cents": float(row["estimated_cost_cents"] or 0),
